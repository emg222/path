           ...
           int* restrict lnew)  // Partial distance at step s+1
{
    ...
    #pragma omp parallel private(tid) shared(l, lnew) reduction(&& : done)
    {
        ...
        //             int a = l[I * nrows + j + (row_offset + i) * n];
        //             for (int k = 0; k < nrows; ++k) {
        //                 int b = l[k + col_offset + (row_offset + j + I * nrows) * n];
        //                 int result_idx = k + col_offset + (row_offset + i + row_offset) * n;
